...
help() {
    ...
    echo "-t, --time            Display activities within a specified time range or for a specific date"
...
time_range() {
    ...
    # If end_time is not provided, set it to the current time
    if [ -z "$end_time" ]; then
        end_time=$(date --iso-8601=seconds)
    elif ! date --date="$end_time" >/dev/null 2>&1; then
        echo "Error: Invalid end time format."
        exit 1
    fi

    log "Activities from $start_time to $end_time:"
    echo -e "\033[1m| TIMESTAMP                   | USER  | ACTIVITY               |\033[0m"
    echo "---------------------------------------------------------------"

    # Execute journalctl command with the given time range
    journalctl --since="$start_time" --until="$end_time" --output=short-iso | while IFS= read -r line; do
...
case $1 in
    ...
    -t|--time)
        if [ -z "$2" ]; then
            echo "Please provide a start time."
            exit 1
        else
            time_range "$2" "$3"
        fi
